...
bool GrayStratify::GSMain(ST_MTPAIR&& stMtPair, ST_MTRESULT* pstMtRet)
{
	bool bRet = true;
...
bool GrayStratify::stratify_one(ST_MTPARAM &param, int *scale, VT_GS &vt_gs)
{
...
bool GrayStratify::InitThresold()
{
...
bool GrayStratify::GetCenter(bool global)
{
	if (global)
	{
...
bool GrayStratify::center_global(wxPoint &pt, ST_MTPARAM &param)
{
...
bool GrayStratify::center_mp(wxPoint &pt, ST_MTPARAM &param, VT_GS &vtgs)
{
	double cent_x = 0.0, cent_y = 0.0;
	int num = 0;
...
double GrayStratify::Similary(ST_GSNODE &gs_a, ST_GSNODE &gs_b)
{
...
double GrayStratify::simi_overlap(int a_x, int a_y, int b_x, int b_y)
{
...
double GrayStratify::simi_intensity(ST_SPOT_NODE &nd_a, ST_SPOT_NODE &nd_b)
{
...
bool GrayStratify::Release()
{
	if (m_nPair > 0)
	{
		...
		if (cnt_p != m_nPair)
		{
			wxASSERT_MSG(false, _T("GrayStratify::Release. Spot pairs' number has error."));
